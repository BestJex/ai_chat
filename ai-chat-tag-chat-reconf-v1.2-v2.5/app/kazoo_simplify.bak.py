...
# 向底层发起请求，并将返回结果写入队列
def _writer(que, kb_id, kb_version, addr, issue):
    addr = 'http://' + addr
    ...
    if res.status_code == 500:
        ...
        if code == 0:
            que.put(ret)
            logger.debug(
                '向%s发送请求成功，向queue写入数据, 写入数据为%s' % (addr, ret))
            # logger.debug(que.empty())
        elif code == 1 and 'score' in ret.keys() and ret['score'] == 0:
            que.put({'not_match': kb_id})
            logger.debug('向%s发送请求未匹配到答案, 知识库为%s, 问题为%s' % (addr, kb_id, issue))
        else:
            que.put({'fail': kb_id})
            logger.debug(
                '向%s发送请求失败, 知识库为%s, 问题为%s, 问答结果为:%s ' % (addr, kb_id, issue, ret))
...
def query_request_z(kb_ids, issue, kb_vers_map):
    ...
    while True:
        if lock_label:
            lock_label = False
            break
    # seed = int(cache.get("top"))
    # temp = (seed + num) % 180
    # cache.set("top", str(temp), timeout=None)
    global top_label
    seed = int(top_label)
    temp = (seed + num) % 180
    top_label = temp
